...
#define S_FUNCTION_NAME  SimvtkColorTransferFunctionMat
...
static void mdlStart(SimStruct *S)
{
   ...
   */
  vtkColorTransferFunction *filter = vtkColorTransferFunction::New();
  ...
  if (numDimensions0 != 2 || (dimensions0[1] != 4 && dimensions0[1] != 6))
    {
    char message[128];
    sprintf(message, "Input should be an Nx4 or Nx6 matrix, was %ix%i.",
            dimensions0[0],
            (numDimensions0 == 2 ? dimensions0[1] : 1));
  ...
  if (dimensions0[1] == 4)
    ...
    for (i=0; i<dimensions0[0]; i++)
      ...
      filter->AddRGBPoint(InputMatrix[0*dimensions0[0] + i],
                          ...
                          InputMatrix[2*dimensions0[0] + i],
                          InputMatrix[3*dimensions0[0] + i],
                          InputMatrix[4*dimensions0[0] + i],
                          InputMatrix[5*dimensions0[0] + i]);
      }
    }

  vtkColorTransferFunction **OutputPort;
  OutputPort = reinterpret_cast<vtkColorTransferFunction**>(ssGetOutputPortSignal(S, 0));
  OutputPort[0] = filter;
}
#endif /*  MDL_START */
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
   ...
   * location of where self indicator will be)
   */
  vtkColorTransferFunction *filter = reinterpret_cast<vtkColorTransferFunction *> (ssGetUserData(S));
...
static void mdlTerminate(SimStruct *S)
{
  ...
  vtkColorTransferFunction *filter =
    reinterpret_cast<vtkColorTransferFunction*> (ssGetUserData(S));
  if (filter != NULL)
    {
    filter->Delete();
    }
