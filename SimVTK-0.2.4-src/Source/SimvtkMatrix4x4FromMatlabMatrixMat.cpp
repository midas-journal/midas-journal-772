...
#define S_FUNCTION_NAME  SimvtkMatrix4x4FromMatlabMatrixMat
...
{
  int nInputPorts  = 1;  /* just 1 input for the matrix from matlab */
  int nOutputPorts = 1;  /* just 1 that will be a pointer to a vtkTransform object created by this class */
...
static void mdlStart(SimStruct *S)
{
  ...
  vtkMatrix4x4 *filter = vtkMatrix4x4::New();
  ...
  if (numDimensions0 != 2 || dimensions0[0] != 4 || dimensions0[1] != 4)
    {
    ssSetErrorStatus(S, "Matrix should be a single 4x4 matrix.");
    return;
    }
  ...
  {
    for (j = 0; j<dimensions0[1]; j++)
    {
    filter->SetElement(j, i, InputMatrix[((i*4)+(j))]);
    }
  }

  vtkMatrix4x4 **OutputPort;
  OutputPort = reinterpret_cast<vtkMatrix4x4**>(ssGetOutputPortSignal(S, 0));
  OutputPort[0] = filter;
}
#endif /*  MDL_START */
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
  int i = 0, j = 0;
  //get the number of inputs that the vtkAlgorithm will use (so know location of where self indicator will be)
  vtkMatrix4x4 *filter = reinterpret_cast<vtkMatrix4x4 *> (ssGetUserData(S));
  ...
  for (i=0; i<dimensions0[0]; i++)
  {
    for (j = 0; j<dimensions0[1]; j++)
    {
    filter->SetElement(i, j, InputMatrix[((i*4)+(j))]);
    }
  }
