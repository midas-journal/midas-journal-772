...
==================
Using SimVTK on Mac OS X
==================

There are two steps that must be followed in order for SimVTK to
function on Mac OS X.

It is imperative that two environment variables are altered to ensure
SimVTK will run. The VTK "lib/vtk-<version>" directory needs to be added
to the DYLD_LIBRARY_PATH environment variable, as this is not done 
through a standard build and install of VTK.

Edit the ~/.bashrc file of the user(s) that will be using SimVTK and
ensure that the following lines are present (using /usr/local as an
example installation directory):

export DYLD_LIBRARY_PATH=/usr/local/lib/vtk-5.2

When running MATLAB under OS X by executing the .app that comes with a
standard MATLAB installation, a script is run to create the MATLAB
instance. When this script runs, the environment variables of the shell 
instance used overwrites the previously made changes made to the 
user's ~/.bashrc file.

To get around this, MATLAB must be invoked from the X11 shell prompt
...
MovingCone.mdl
   ...
   Displays a cone that has its Center parameter connected to a sinusoid
   source, plus a cylinder that is used as a target for the cone.
